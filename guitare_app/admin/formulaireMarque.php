...
if(isset($_GET['idMarque'])){
    ...
    if($_GET['idMarque'] !== null)
    {
        $marque=MarqueRepo::getMarque($_GET['idMarque']);
        $idMarque=getIdObject($marque);
        $message="modification de ".getNomObject($marque)." :";
        $action='Modification ';
    }
...
if(isset($_POST["submit"])){
    ...
    if(isset($_POST['nomMarque']) && !empty($_POST['nomMarque'])){

        if($_POST['id'] > 0){
        $upMarque=MarqueRepo::getMarque($_POST['id']);
        $upMarque->setNomMarque(cleanInput($_POST['nomMarque']));
        MarqueRepo::setMarque($upMarque);
        header('location:marque.php');
    }

    else{
        if(!existInBase(cleanInput($_POST['nomMarque']),'Marque')){

        $newMarque=new Marque(0,cleanInput($_POST['nomMarque']));
        MarqueRepo::addMarque($newMarque);
        header('location:marque.php?flag=1');
        }
